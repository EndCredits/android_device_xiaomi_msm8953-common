...
if [ "$DEVICE" = "mido" ]; then
    VENDOR_BLOB_ROOT="${ANDROID_ROOT}"/vendor/"${VENDOR}"/"${DEVICE}"/proprietary

    # Camera configs
    sed -i "s|/system/etc/camera|/vendor/etc/camera|g" "${VENDOR_BLOB_ROOT}"/vendor/lib/libmmcamera2_sensor_modules.so

    # Camera socket
    sed -i "s|/data/misc/camera/cam_socket|/data/vendor/qcam/cam_socket|g" "${VENDOR_BLOB_ROOT}"/vendor/bin/mm-qcamera-daemon
    ...
    for CAMERA_LIB in libmmcamera2_cpp_module.so libmmcamera2_dcrf.so libmmcamera2_iface_modules.so libmmcamera2_imglib_modules.so libmmcamera2_mct.so libmmcamera2_pproc_modules.so libmmcamera2_q3a_core.so libmmcamera2_sensor_modules.so libmmcamera2_stats_algorithm.so libmmcamera2_stats_modules.so libmmcamera_dbg.so libmmcamera_imglib.so libmmcamera_pdafcamif.so libmmcamera_pdaf.so libmmcamera_tintless_algo.so libmmcamera_tintless_bg_pca_algo.so libmmcamera_tuning.so; do
        sed -i "s|/data/misc/camera/|/data/vendor/qcam/|g" "${VENDOR_BLOB_ROOT}"/vendor/lib/${CAMERA_LIB}
    done
    
    # Camera debug log file
    sed -i "s|persist.camera.debug.logfile|persist.vendor.camera.dbglog|g" "${VENDOR_BLOB_ROOT}"/vendor/lib/libmmcamera_dbg.so

    # Camera graphicbuffer shim
    patchelf --add-needed libui_shim.so  "${VENDOR_BLOB_ROOT}"/vendor/lib/libmmcamera_ppeiscore.so

    # Wcnss_service - libqmiservices_shim
    patchelf --add-needed "libqmiservices_shim.so" "${VENDOR_BLOB_ROOT}"/vendor/bin/wcnss_service
    sed -i "s|dms_get_service_object_internal_v01|dms_get_service_object_shimshim_v01|g" "${VENDOR_BLOB_ROOT}"/vendor/bin/wcnss_service
fi
